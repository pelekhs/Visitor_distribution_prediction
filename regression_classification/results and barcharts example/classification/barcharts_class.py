...
#results = (csv.iloc[:5,1:].values).astype(float)

#classification csv
csv =pd.read_csv('classification_accuracies_15_15_SHIFT_1.csv') 
results1 = (csv.iloc[:5,1:].values)
results=[]
for i in range(5):
    results.append([results1[i][x][:-1] for x in range(len(results1[i]))])
results=(np.asarray(results).astype(float))

#common
clusters= ['A','B','C','D','E','F']
cofs=[0,1,2,3]
ests = ['Random Forest','KNN','SVM','Bayes classifier','Deep Network']
cof=[]
cluster=[]
for cl in clusters:
    for i in cofs:
...
for [mean,name] in list(zip([cof_mean,cluster_mean],["Combination of Features","Area of Interest"])):
    ...
    plt.savefig("Classifier_accuracies_per_ "+name+".png")
    plt.show()
