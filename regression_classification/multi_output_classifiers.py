...
# =============================================================================
    ...
    def create_fit_estimator(estim, X_train_, y_train_, X_test_, y_test_, n_it):
        ...
        for combo in combos:
            ...
            pipe_svm = Pipeline([('scl', StandardScaler()), ('class', MultiOutputClassifier(estim["estimator"]))])

            clf = RandomizedSearchCV(estimator = pipe_svm,iid =True, cv=5, scoring = acc,
                                 param_distributions=estim["param_dist"], n_iter=n_it)
            try:
            ...
            accuracy[combo] = np.sum(np.equal(y_test_, y_pred))/float(y_test_.size)
    ...
    RF_param_dist = {"class__estimator__estimator__n_estimators": [int(x) for x in np.linspace(start = 10, stop = 40, num = 10)],
                     "class__estimator__estimator__max_depth" : [int(x) for x in np.linspace(3, 20, num = 11)],
                     "class__estimator__estimator__min_samples_leaf": [1, 2, 4]}

    KNN_param_dist = {"class__estimator__estimator__n_neighbors" : range(1, 31),
                      "class__estimator__estimator__weights" : ['uniform', 'distance']}

    SVC_param_dist = {"class__estimator__estimator__C": stats.uniform(1e-1, 3),
                      "class__estimator__estimator__gamma": stats.uniform(1e-2, 100)}
...
#                               X_test_ = X_test, y_test_ = y_test, n_it=iterat), estimators)
    ...
    for keys in estimators:
        skata[keys] = create_fit_estimator(estimators[keys], X_train_ = X_train, y_train_ = y_train,
                               X_test_ = X_test, y_test_ = y_test, n_it=iterat)

# =============================================================================
# WRITE METRICS+RESULTS
# =============================================================================

    accuracies = pd.DataFrame(index=["+time","+time_pops","+time_temp/cond","+time_pops_temp/cond"], columns = list(estimators.keys()))

    for keys in skata:
        for combinations in skata[keys]:
            accuracies[keys][combinations] = skata[keys][combinations]

#    print ("ACC:", np.sum(np.equal(y_test, y_pred))/float(y_test.size))

    metrics.to_csv(directory+'metrics_'+parameters['dir'][:-3]+'_SHIFT_'+str(shift_for)+'.csv')
    accuracies.to_csv(directory+'multioutput_'+parameters['dir'][:-3]+'_SHIFT_'+str(shift_for)+'.csv')
